...
const eventsCollection = (firestore: Firestore) =>
  ...
  const payload: DocumentData = {
    ...
    date_start:
      data.date_start instanceof Date
        ? Timestamp.fromDate(data.date_start)
        : data.date_start,
    date_end:
      data.date_end instanceof Date
        ? Timestamp.fromDate(data.date_end as Date)
        : data.date_end ?? null,
...
export async function getEventsByUser(firestore: Firestore, userId: string) {
  ...
  const events: StoredEvent[] = snap.docs.map((d) => ({
    id: d.id,
    ...(d.data() as EventData),
  }));
  return events;
...
export function subscribeToUserEvents(
  firestore: Firestore,
  userId: string,
  callback: (events: StoredEvent[]) => void
) {
  ...
  const unsub = onSnapshot(q, (snapshot) => {
    const events: StoredEvent[] = snapshot.docs.map((d) => ({
      id: d.id,
      ...(d.data() as EventData),
    }));
    callback(events);
...
/**
 * Get a single event by ID. Returns null if not found or soft-deleted.
 */
export async function getEventById(firestore: Firestore, eventId: string) {
  const ref = doc(firestore, "events", eventId);
  const snap = await getDoc(ref);
  if (!snap.exists()) return null;
  const data = snap.data() as EventData;
  if (data.is_soft_deleted) return null;
  return { id: snap.id, ...(data as EventData) } as StoredEvent;
}
